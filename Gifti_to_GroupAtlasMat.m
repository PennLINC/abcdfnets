...
template='/cbica/projects/abcdfnets/nda-abcd-s3-downloader/outdir164/derivatives/abcd-hcp-pipeline/sub-NDARINV0314RN9P/ses-baselineYear1Arm1/func/sub-NDARINV0314RN9P_ses-baselineYear1Arm1_p2mm_masked.dtseries.nii'
...
for i=1:17
	...
	networkFPL=strjoin(['~/conversion_tests/resamp_32k_network' string(i) '_L.32k_fs_LR.func.gii'],'');
	%networkLeft=gifti(char(networkFPL));
	networkFPR=strjoin(['~/conversion_tests/resamp_32k_network' string(i) '_R.32k_fs_LR.func.gii'],'');
	%networkRight=gifti(char(networkFPR));
	% combine hemispheres
	combinedFN=[ProjectFolder '/Group_AtlasLoading_Network_' num2str(i) '.dscalar.nii'];
...
outFP=[ProjectFolder '/SingleParcellation/RobustInitialization_Cifti_Surf/initResamp.mat'];
save(outFP,'initV');
